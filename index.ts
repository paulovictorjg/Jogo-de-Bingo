...
const colunaNumeros: number[][] = [[], [], [], [], []];
const numerosPorColuna: number = 15;
...
const celulasNumeros: NodeListOf<Element> = document.querySelectorAll(
  '#tabelaBingo tr:not(:first-child) td'
);
...
function verificarNumeroTabela(numero: number): boolean {
  for (let i = 0; i < celulasNumeros.length; i++) {
...
function sortear(): boolean | void {
  const numero: number = sortearNumero();
...
const botaoSortear: HTMLElement | null =
  document.querySelector('#botaoSortear');
botaoSortear!.addEventListener('click', sortear);

// Função para o botão "Nova Cartela"
function novaCartela(): void {
...
const botaoNovaCartela: HTMLElement | null = document.querySelector('#novaCartela');
botaoNovaCartela!.addEventListener('click', novaCartela);

// Função para verificar se há bingo na cartela
function verificarBingo(): boolean {
  const linhas: NodeListOf<Element> = document.querySelectorAll('#tabelaBingo tr:not(:first-child)');

  for (let i = 0; i < linhas.length; i++) {
    const celulas: NodeListOf<Element> = linhas[i].querySelectorAll('td');

    if (
      Array.from(celulas).every((celula: Element) =>
  ...
  for (let i = 0; i < 5; i++) {
    const celulas: NodeListOf<Element> = document.querySelectorAll(
      `#tabelaBingo tr:not(:first-child) td:nth-child(${i + 1})`
    );
  ...
  const diagonal1: Element[] = [];
  const diagonal2: Element[] = [];
  
  for (let i = 0; i < 5; i++) {
    diagonal1.push(
      document.querySelector(
        `#tabelaBingo tr:nth-child(${i + 2}) td:nth-child(${i + 1})`
      )!
    );
    
    diagonal2.push(
      document.querySelector(
        `#tabelaBingo tr:nth-child(${i + 2}) td:nth-child(${5 - i})`
      )!
    );
   ...
   if (
     diagonal1.every((celula: Element) => celula.classList.contains('marcado')) ||
     diagonal2.every((celula: Element) => celula.classList.contains('marcado'))
   ) {
     return true;
   }
   
   // Verificar as quatro células das pontas
   const pontas: Element[] = [
   ...
   if (pontas.every((celula: Element) => celula.classList.contains('marcado'))) {
     return true;
   }
   
   // Verificar todas as células da tabela de bingo
   const todasCelulas: NodeListOf<Element> = document.querySelectorAll('#tabelaBingo td');
   
   if (
     Array.from(todasCelulas).every((celula: Element) =>
       celula.classList.contains('marcado')
     )
   ) {
...
const botaoVerificarBingo: HTMLElement | null = document.querySelector('#verificarBingo');

botaoVerificarBingo!.addEventListener('click', function (): void {
